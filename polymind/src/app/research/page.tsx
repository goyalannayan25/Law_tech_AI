...
export default function ResearchPage() {
  ...
  const { setTopic, setQuestions } = useDeepResearchStore();

  // Auto-start research if query parameter is present
  useEffect(() => {
    const autoStartResearch = async () => {
      if (query) {
        try {
          ...
          console.error("Error auto-starting research:", error);
        }
      }
    };

    autoStartResearch();
  }, [query, setTopic, setQuestions]);
    ...
    <main className="min-h-screen w-full flex flex-col items-center justify-start gap-8 py-16">
      ...
      <div className="flex flex-col items-center gap-4 relative w-full">
        ...
        <p className="text-gray-600 text-center max-w-[90vw] sm:max-w-[50vw]">
          {query ? 'Processing your request...' : 'Enter a topic and answer a few questions to generate a comprehensive research report.'}
        </p>
      </div>
